...
(function (global, $) {
	...
	var modal = {
		...
		fillSpeaker: function (event) {
		    event.preventDefault();
		    if (jgn("#eventSpeakerAnchor").length == 0) {
		        jgn("#modal-AGM div.modal-content").html("...loading");
		        jgn.ajax({
		            url: document.location.origin + "" + document.location.pathname + "?speakerId=" + event.data.param1,
		        }).done(function (data) {
		            var obj = JSON.parse(data);
		            var speakerTemplate = "<div class='speakerTeaser modal'><img style='float: left;' src='{0}'><div class='speakerDetails'><h2>{1}</h2><h3 class='breath'>{2}</h3></div><p>{3}</p></div>";
		            speakerTemplate = speakerTemplate.replace(/\{0\}/g, obj.Picture);
		            speakerTemplate = speakerTemplate.replace(/\{1\}/g, obj.FirstName + " " + obj.LastName);
		            speakerTemplate = speakerTemplate.replace(/\{2\}/g, obj.Title);
		            speakerTemplate = speakerTemplate.replace(/\{3\}/g, obj.Biography);
		            jgn("#modal-AGM div.modal-content").html(speakerTemplate);

		        });

		        jgn("a[href='#modal-AGM']")[0].click();
		    } else {
		        var speakerPage = window.location.href.split("?", 1)[0];
		        speakerPage = speakerPage.replace("#modal-Events", "");
		        speakerPage = speakerPage.replace("#!", "");
		        jgn("#modal-Events div.modal-content").html("...loading");
		        jgn.ajax({
		            url: speakerPage + "?speakerId=" + event.data.param1,
		        }).done(function (data) {
		            var obj = JSON.parse(data);
		            var speakerTemplate = "<div class='speakerTeaser modal'><img style='float: left;' src='{0}'><div class='speakerDetails'><h2>{1}</h2><h3 class='breath'>{2}</h3></div><p>{3}</p></div>";
		            speakerTemplate = speakerTemplate.replace(/\{0\}/g, obj.Picture);
		            ...
		            speakerTemplate = speakerTemplate.replace(/\{2\}/g, obj.Title);
		            speakerTemplate = speakerTemplate.replace(/\{3\}/g, obj.Biography);
		            jgn("#modal-Events div.modal-content").html(speakerTemplate);

		        });

		        jgn("a[href='#modal-Events']")[0].click();
		    }
		},
		fillSponsor: function (event) {
			event.preventDefault();
			jgn("#modal-AGM div.modal-content").html("...loading");
			jgn.ajax({
				url: document.location.origin + "" + document.location.pathname + "?sponsorId=" + event.data.param1,
			}).done(function(data){
				var obj = JSON.parse(data);
				var speakerTemplate = "<div class='speakerTeaser modal'><div class='sponsorDetails'><div><a href='{2}' target='_blank'><img class='sponsorImg' src='{0}'></a></div><div><h2>{1}</h2><a href='{2}' target='_blank'>{2}</a></div></div><p>{3}</p></div>";
				speakerTemplate = speakerTemplate.replace(/\{0\}/g,obj.Picture);
				speakerTemplate = speakerTemplate.replace(/\{1\}/g,obj.Title);
				speakerTemplate = speakerTemplate.replace(/\{2\}/g,obj.URL);
				if (obj.Description != null)
				{
					speakerTemplate = speakerTemplate.replace(/\{3\}/g,obj.Description);
				}
				else{
					speakerTemplate = speakerTemplate.replace(/\{3\}/g,"");
				}
				jgn("#modal-AGM div.modal-content").html(speakerTemplate);
		...
		fillVideo: function (event) {
		    event.preventDefault();
		    var videoSelector = "div.agmVideo[videoid='" + event.data.param1 + "'] a";
		    var videoURL = jgn(videoSelector).attr("videourl");
		    jgn("#modal-AGM div.modal-content").html("...loading");
		    var videoTemplate = "<video class='video-js' id='my-video' preload='auto' controls='controls'><source src='{0}' type='video/mp4'><div class='vjs-no-js'><p>Please consider upgrading to a web browser that supports HTML5 video</p></div></video>";
		    videoTemplate = videoTemplate.replace(/\{0\}/g, videoURL);
		    jgn("#modal-AGM div.modal-content").html(videoTemplate);

		    jgn("a[href='#modal-AGM']")[0].click();
		    jgn('div.modal-content video.video-js').on("play", function () {
		        console.log('...::: Videos Played (This event will track videos played in modal popup) :::...');
		        var eventAction = jgn(this)[0].currentSrc;
		        var eventLabel = GetCurrentRequestUri();
		        dataLayer.push({
		            'event': 'eventGA',
		            'eventCategory': 'Videos Played',
		 ...
		 */
		init: function () {
		    var template = "<section class='modal--show' id='modal-AGM' tabindex='-1' role='dialog' aria-labelledby='modal-label' aria-hidden='true'><div class='modal-inner'><div class='modal-content'>...loading</div></div><a href='#!' class='modal-close' title='Close this modal' data-close='Close'data-dismiss='modal'>?</a></section><section class='modal--show' id='modal-Events' tabindex='-1' role='dialog' aria-labelledby='modal-label' aria-hidden='true'><div class='modal-inner'><div class='modal-content'>...loading</div></div><a href='#!' class='modal-close' title='Close this modal' data-close='Close'data-dismiss='modal'>?</a></section>";


		    jgn("body").prepend(template);
			jgn(function(){
				jgn("#videoFilter").change(function() {
					if (jgn("#videoFilter option:selected").text() == "All videos")
					{
						jgn("div.videosContainer").fadeOut(1500, function(){jgn("div.agmVideo").show(); jgn("div.videosContainer").fadeIn(1500);});
					}
					else{
						var currentOption = "div.agmVideo[videocategory='"+jgn("#videoFilter option:selected").text()+"']";
						var otherOptions = "div.agmVideo[videocategory!='"+jgn("#videoFilter option:selected").text()+"']";
						jgn("div.videosContainer").fadeOut(1500, function(){jgn(currentOption).show(); jgn(otherOptions).hide(); jgn("div.videosContainer").fadeIn(1500);});

					}
				});
				jgn(".AGMHomeRow video.video-js").attr("controls","");
				jgn("a[speakerid]").each(function () { jgn(this).click({ param1: jgn(this).attr("speakerid") }, modal.fillSpeaker) });
				jgn(".speakerTeaser").each(function () { jgn(this).click({ param1: jgn(this).attr("id") }, modal.fillSpeaker) });
				jgn("div.sponsor").each(function(){jgn(this).click({param1: jgn(this).attr("sponsorid")},modal.fillSponsor)});
				jgn("div.agmVideo img").each(function(){jgn(this).click({param1: jgn(this).parent().attr("videoid")},modal.fillVideo)});
				jgn(document).on('cssmodal:hide', function (event) {
					jgn("#modal-AGM div.modal-content").html("...loading");
				});
			});
			/*
